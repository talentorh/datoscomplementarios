...
extract($_POST);
$sql = $conexion->prepare("UPDATE datospersonales set cargodocumento = :cargodocumento where curp = :curp and fechainicio between '2023-01-31' and '2023-12-31'");
        $sql->execute(array(
            ':cargodocumento'=>1,
            ':curp' =>$curp
        ));
...
if ($validacurp != '') {
    ...
    :nombre, :appaterno, :apmaterno, :estado, :delegacion, :localidad, :colonia, :calle, :numexterior, :numinterior, :codigopostal, :fechanacimiento, :entidadnacimiento, :rfc, :sexo, :cartanaturalizacion, :telefonocasa, :telefonocelular, :otrotelefono, :correoelectronico,:fechaactualizo)");
        ...
        if ($_FILES["documentocurp"]["size"] == 0) {
            ...
            $from = '../../reestructuracionpaginawebhraei/documentos/' . $compdomicilio . $curp . '/';
            $to = '../documentos/'. $compdomicilio. $curp . '/';
            mkdir($ruta);
    ...
    /*incializa edicion de comprobante de domicilio*/
        ...
        if ($_FILES["archivosuperior"]["size"] == 0) {
        ...
        if ($_FILES["archivocedulasuperior"]["size"] == 0) {
