...
$statement = $conexion->prepare('SELECT id_datopersonal,correoelectronico, rfc FROM datospersonales WHERE correoelectronico= :correoelectronico and acceder = :acceder');
    ...
    $sql = $conexionSeleccion->prepare("SELECT datosActualizados, rechazoContratacion, id_datopersonal from datospersonales where correoelectronico = :correoelectronico");
        $sql->execute(array(
            ':correoelectronico'=>$usernameSesion
        ));
        ...
        $rechazo = $validacion['rechazoContratacion'];
...
$dataRegistro= $query->fetch();
    ...
    }else if($actualizo == 1 and $rechazo == 1){
...
}else if($actualizo == 1 and $rechazo == 0){
...
$dataRegistro= $query->fetch();
require 'frontend/datosActualizadosC.php';
    }else if($actualizo == 3 and $rechazo == 0){
